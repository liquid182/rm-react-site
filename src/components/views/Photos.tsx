...
export class Photos extends React.Component<{}, IAboutState> {
  ...
  private initSlides = () => {
    GDrive.getFileList(GOOGLE_API_KEY, GOOGLE_FOLDER_ID).then(
      (response: IGDrive) => {
        let slides: string[] = response.files.map(
          GDrive.mapGDriveFileToURLStringArray
        );
